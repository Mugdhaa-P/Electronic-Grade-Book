...
class Student:
    def __init__(self,idNumber,assignmentScoreList = {}): #for every student, an empty dictionary for all the assignments as the key names will be created
    ...
    def getScore(self, assignmentName):
        return int(self._assignmentScoreList[assignmentName][0])
    ...
    def addAssignment(self,assignment):
        ...
        self._assignmentScoreList[des] = [score, total]


    def changeScore(self, assignmentName, score):
        if assignmentName in self._assignmentScoreList:
            self._assignmentScoreList[assignmentName][0] = float(score)
        else:
            pass
...
class Gradebook:
    ...
    def addAssignment(self, idNum, score:Assignment):
        for student in self._students:
            print('printing student objects: ',self._students)
            idNumber = student.getId()
            print('idNum: ',idNumber)
            print('student object before: ', student)
            if idNumber == idNum:
                print("ID matched")
                des = score.getDescription()
                print('des is:',des)
                personalList = self._assignmentScoreList
                print('PersonalList is: ', personalList)
                personalList[des] = score
                #student.addAssignment(score)
                print('student object after adding: ', student)
                return
            else:
    ...
    def classAverage(self):
        ...
        for student in self._students:
            for assignment in student._assignmentScoreList:
                s = student._assignmentScoreList[assignment].getScore()
                t = student._assignmentScoreList[assignment].getTotal()
                self._score += s
                self._total += t
